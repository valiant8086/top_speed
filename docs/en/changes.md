...
- macOS: Fixed keys getting stuck or going dead while driving with VoiceOver running. Steering could stop even though you were still holding the key, and letting go of every arrow and pressing one again was the only way to get it back; a key could also stay on after you let go, leaving the throttle open or the car turning by itself. VoiceOver watches certain arrow combinations for its own shortcuts and takes those key presses on their way to the game, so the game was left believing a key was down when it was up, or up when it was down. It now reads the arrow keys from the keyboard itself, the way it always has on Windows. Ovals, where a turn is held for most of a lap, were the worst affected.
