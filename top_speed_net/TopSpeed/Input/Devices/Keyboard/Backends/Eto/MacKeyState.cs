...
namespace TopSpeed.Input.Devices.Keyboard.Backends.Eto
{
    ...
    // who consumed the event on its way to the window. The Eto device is event-driven, so a release it
    // never receives leaves that key held forever — VoiceOver causes exactly that, because it watches
    // the arrow chords (Left+Right toggles QuickNav, Up+Left and Up+Right turn the rotor) and eats
    // their key-ups. The visible result while driving is a throttle or a steer that stays on after the
    // key is let go. Windows never had this because DirectInput polls the device instead of counting
    // events; this is what lets the Eto backend do the same.
    //
