...
### Game Changes
- Linux and macOS: The game now reads the keyboard by which key you pressed rather than by the character your layout produced, the same way it has always worked on Windows. Shift with Tab, Shift with 1 and Shift with the backtick can be used as shortcuts now; they used to arrive as something the game could not make sense of, so they did nothing at all. Keys also no longer stick down. A key being let go could go unnoticed, leaving the game acting as though you were still holding it until you pressed and released it again on its own. The game window, typing, the clipboard and the file pickers all come from the same place now as well, which is what the phone and tablet versions have been using all along.
...
- Linux: The game window now opens and takes the keyboard on Wayland desktops, such as the standard desktop on Ubuntu. The window was listed when you switched windows but never actually appeared, so it could not be focused and your keys went to whatever was behind it, while the game itself carried on playing its startup sound and speaking.
...
- DECIDE BEFORE RELEASE, then delete this line: on Wayland the game no longer has SDL draw its own title bar, which stops GTK, ATK, the accessibility bridge and the canberra sound module from being loaded into the game alongside the screen reader, and stops the warnings GTK printed about its own objects while it was. Nothing about this is observable without sight: the window is still named in the window switcher, and Alt+Space still maximizes and closes it. What goes is the drawn title bar, so a sighted player has no strip to drag the window by or buttons to click, and would use Alt+Space or Super and drag instead. Keep it and this line goes away with nothing said to players, or back it out and accept the warnings.
